/*
 * robotron_pad_v2 - serial controller firmware for the direct-wired Xbox 360 pad.
 *
 * Shield revision v2 - Eric's board, the clean one. Uses D3..D8 and D10..D13;
 * D9 is unused, same pin set as v1 but a different order.
 *
 * Pin map DERIVED, not probed. Eric's rig has run the legacy serial_pin_monitor
 * firmware correctly for years, and that firmware plus the byte layout below
 * pins every channel down exactly. Cross-check: the derivation says D3 presses
 * D-pad up, and Strider independently observed D3 hitting up on this board
 * where it hits Back on v1.
 *
 * The legacy sketch got the right result from misleading names: it called D10
 * xButton and D3 leftButton, but its myPins[] order meant D10 really pressed A
 * and D3 really pressed D-pad up. The names below say what each pin does.
 *
 * If Eric's Uno turns out to run a locally modified sketch, this derivation
 * moves. Confirm in about a minute: flash arduino/pin_probe, run
...
// Derived from the legacy firmware this shield has run correctly for years.
