...
| Sep 16 2026 | Console round 16: corrected forward prediction (2.5 ticks) and an uncompressed capture format | Neither changed anything: same deaths per wave, same 3-decision delay. The delay is in the control adapters or the console's output, and the bot cannot compensate it. | Next: bypass the two X-Arcade adapters (optoisolators straight onto a wired 360 pad). |
